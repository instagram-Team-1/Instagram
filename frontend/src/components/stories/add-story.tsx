...
import AddStorySVG from "../../../public/svg-components/addstorysvg";
import updateUserStory from "@/utils/handle-update-user-story";
import handleUploadImage from "@/utils/handle-upload-image";
...
import handleRemoveStory from "@/utils/handle-remove-story";
...
function AddStory() {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!username) {
      console.warn("⛔ Username is undefined. Skipping upload.");
      return;
    }
    ...
    handleUploadImage({
      ...
      setLoading,
      setAddPhoto,
      handleImgURLFunction: updateUserStory,
    });
  };
          ...
          {loading ? (
            <div className="animate-spin rounded-full bg-[#000000de] p-2">
              <picture>
                <img
                  className="h-10 w-10"
                  src="frontend/public/img/instaLogo.png"
                  alt="instagram loading"
                />
              </picture>
            </div>
          ) : (
            <div className="w-[400px] flex-col rounded-xl bg-white dark:border dark:border-stone-300 dark:bg-[#000000]">
